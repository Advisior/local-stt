...
<dict>
    ...
    <array>
        <string>/path/to/claude-stt</string>
        <string>start</string>
    </array>
    <key>RunAtLoad</key>
    <true/>
...
Replace `/path/to/claude-stt` with the output of `which claude-stt`.
...
#table(
  ...
  [Wrong language output], [Add `language = "de"` to config],
  [Slow transcription], [Switch to `small` model, or ensure no heavy GPU tasks],
  [Hotkey doesn't work], [Quit Wispr Flow or other STT tools],
  [Python 3.14 error], [Use Python 3.12: `python3.12 -m venv .venv`],
  ["pynput unavailable"], [Grant Accessibility: System Settings > Privacy & Security > Accessibility],
)
...
Look for:
- `Audio input: Your Microphone` -- correct mic selected
- `Transcribing audio (... samples, -20.5 dB)` -- good signal (-30 to -10 dB)
